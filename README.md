...
Official server-side client for image, PDF and video watermarking. .NET 10. MIT licensed.
...
`EmbedImageAsync` / `DetectImageAsync`, `EmbedDocumentAsync` / `DetectDocumentAsync`, `EmbedVideoAsync` / `DetectVideoAsync`. Each embed method takes file bytes, JSON metadata and request options; detection takes bytes and options.

Resume saved jobs with `GetEmbedResultAsync(requestId)` / `GetDetectionResultAsync(requestId)`.
...
```sh
dotnet test tests/Contract.csproj
```
